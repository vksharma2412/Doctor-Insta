...
class TutorSlotBooking extends React.Component {
    ...
    hadnleConfirm = (dateAndTime) => {
        ...
        if (this.state.dateTimeDecider.localeCompare(Constant.dateTime.date) == 0) {
            ...
            let selectedDate = moment(new Date(dateAndTime), 'YYY-MM-DD').format('YYYY-M-DD')
            ...
            this.setState({ timeStamp: dateAndTime, visibility: false, start_time: '', end_time: '', time_unit: '' })
        }
        if (this.state.dateTimeDecider.localeCompare(Constant.dateTime.time) == 0) {
            let selectedDate = moment(new Date(dateAndTime), 'YYY-MM-DD').format('HH:mm:ss').toString()
            let addedDate = moment(this.state.timeStamp, 'YYY-MM-DD').format().toString()
            let currentDate = moment().format('YYYY-MM-DD hh:mm A')
            console.log("moment(moment(this.state.timeStamp, 'YYY-MM-DD').format('YYYY-MM-DD') + selectedDate)==>" + moment(moment(this.state.timeStamp, 'YYY-MM-DD')).format('YYYY-MM-DD'))



            // console.log("selectedDate  ==>> " + moment(addedDate + " " + selectedDate, 'YYYY-MM-DD HH:mm'))
            // console.log("selectedDate  ==>> " + moment(currentDate, 'YYYY-M-DD HH:mm '))
            // let curD = moment(currentDate, 'YYYY-M-DD HH:mm ')
            // let selD = moment(addedDate + " " + selectedDate, 'YYYY-MM-DD HH:mm')



            console.log(moment(addedDate + " " + selectedDate, "YYYY-MM-DD, HH:mm").isSameOrAfter(moment()))
            console.log("moment(addedDate  + selectedDate)" + moment(addedDate + " " + selectedDate, "YYYY-MM-DD, HH:mm"))
            console.log("moment()" + moment())

            if (!moment(addedDate + " " + selectedDate, "YYYY-MM-DD, HH:mm").isSameOrAfter(moment())) {
                this.setState({ visibility: false })
                showToast("Please select future Time")
                return
            }
            const timeAndDate = moment(dateAndTime)
            console.log('timeAndDate' + moment(new Date(timeAndDate).getTime()).add(1, 'hour').format('A'))
            this.setState({ start_time: moment(timeAndDate).toISOString(), end_time: moment(moment(new Date(timeAndDate).getTime()).add(1, 'hour')).toISOString(), time_unit: moment(new Date(timeAndDate).getTime()).add(1, 'hour').format('A'), visibility: false })
        }
    }
...
export default TutorSlotBooking;
